...
const SKILL_META: Record<string, {
  ...
  sentiment: {
    ...
    capability: 'Returns JSON with overall sentiment, score (-1 to +1), detected emotions, confidence, and reasoning.',
  ...
  sql: {
    ...
    capability: 'Converts plain-English queries to valid PostgreSQL. Returns the query, explanation, and assumptions.',
  ...
  chart: {
    ...
    capability: 'Accepts data descriptions or tables. Returns a complete Chart.js config JSON.',
  ...
  extract: {
    ...
    capability: 'Identifies and extracts entities, dates, amounts, and relationships. Returns clean JSON.',
    inputLabel: 'Paste document text or HTML',
    exampleInput: 'Invoice from Acme Corp. Date: January 15 2026. Due: February 1 2026. Total: $4,536.00.',
  ...
  'legal-review': {
    ...
    capability: 'Reviews contracts for liability traps, auto-renewal clauses, IP ownership issues, and missing protections.',
    inputLabel: 'Paste contract text or clauses to review',
    exampleInput: 'This agreement shall automatically renew for successive one-year terms unless either party provides sixty (60) days written notice prior to renewal.',
  ...
  'fact-check': {
    ...
    capability: 'Takes a list of claims and returns a verdict (VERIFIED / UNVERIFIED / DISPUTED) with reasoning.',
    ...
    exampleOutput: '✅ **VERIFIED:** USDC is issued by Circle Internet Financial.\n✅ **VERIFIED:** Arc Testnet chain ID is 1111.\n❌ **DISPUTED:** Ethereum mainnet launched July 30, 2015 — not 2013.',
    acceptsFiles: true,
      ...
      <div className="flex gap-0.5">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="w-2 h-2 rounded-full"
            style={{ background: i < Math.round(score * 5) ? 'var(--accent)' : 'var(--border-accent-dim)' }} />
        ))}
      </div>
  ...
  if (notFound) {
    return (
      <div className="max-w-2xl mx-auto px-6 py-20 text-center">
        ...
        <Link href="/marketplace" className="text-[var(--accent)] text-sm mt-4 block">← Back to Marketplace</Link>
      </div>
    );
  }
    ...
    <div className="max-w-2xl mx-auto px-6 py-10">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="flex items-center gap-1 text-xs font-mono">
                  ...
                  <span style={{ color: isActive ? 'var(--accent)' : 'var(--text-5)' }}>{agent.status}</span>
                </span>
              </div>
            </div>
            <div className="text-right shrink-0">
              <div className="text-2xl font-bold font-mono text-[var(--text-1)]">${agent.price_usdc}</div>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.07 }}
        ...
        {[
          ...
          { label: 'Total Earned', value: `$${agent.total_earned.toFixed(4)}`, color: agent.total_earned > 0 ? 'var(--accent)' : undefined },
        ].map((stat, i) => (
          <div key={i} className="rounded-xl border border-[var(--border-accent-dim)] bg-[var(--surface)] px-4 py-3 shadow-sm">
            <div className="text-[10px] font-mono text-[var(--text-4)] uppercase tracking-wide mb-1">{stat.label}</div>
            {stat.quality !== undefined
              ? <QualityBar score={stat.quality} />
              : <div className="text-sm font-bold font-mono" style={{ color: stat.color ?? 'var(--text-2)' }}>{stat.value}</div>
            }
          </div>
        ))}
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12 }}
        ...
        <div className="px-5 py-3 border-b border-[var(--border-subtle)]">
          <span className="text-xs font-mono text-[var(--text-4)] uppercase tracking-wide">Example</span>
        </div>
        <div className="p-5 space-y-4">
          <div>
            <div className="text-[10px] font-mono text-[var(--text-5)] uppercase tracking-wide mb-1.5">Input</div>
          ...
          <div>
            <div className="text-[10px] font-mono text-[var(--text-5)] uppercase tracking-wide mb-1.5">Output</div>
